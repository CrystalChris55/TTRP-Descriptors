...
function DescriptorCreateModal:initialise()
    ...
    self.title = ISTextEntryBox:new(self.title, self:getWidth() / 2 - ((self:getWidth() - 20) / 2), 20,
        self:getWidth() - 20, 2 + self.fontHgt + 2)
    ...
    self.entry = ISTextEntryBox:new(self.defaultEntryText, self:getWidth() / 2 - ((self:getWidth() - 20) / 2),
        self.title:getBottom() + 4, self:getWidth() - 20, height)
    ...
    if not self.locked then
        self.deleteButton = ISButton:new(self.entry.x, self.entry:getBottom() + 4, btnHgt2, btnHgt2, '', self,
            DescriptorCreateModal.onClick)
        ...
        self.lockButton = ISButton:new(self.deleteButton.x + self.deleteButton.width + 3, self.deleteButton.y, btnHgt2,
            btnHgt2, '', self, DescriptorCreateModal.onClick)
    ...
    if self.numberOfPages > 1 then
        self.pageLabel = ISLabel:new(self.entry.x + self.entry.width, self.entry:getBottom() + 4, FONT_HGT_SMALL,
            getText('IGUI_Pages') .. self.currentPage .. '/' .. self.numberOfPages, 1, 1, 1, 1, UIFont.Small, false)
        self.pageLabel:initialise()
        self.pageLabel:instantiate()
        self:addChild(self.pageLabel)

        self.nextPage = ISButton:new(self.width / 2 + 3, self.pageLabel.y, btnHgt2, btnHgt2, '>', self,
            DescriptorCreateModal.onClick)
        ...
        self.previousPage = ISButton:new(self.nextPage.x - btnHgt2 - 3, self.nextPage.y, btnHgt2, btnHgt2, '<', self,
            DescriptorCreateModal.onClick)
    ...
    self.yes = ISButton:new((self:getWidth() / 2) - btnWid - 5, bottom + 5, btnWid, btnHgt, getText('UI_Ok'), self,
        DescriptorCreateModal.onClick)
    ...
    self.no = ISButton:new((self:getWidth() / 2) + 5, bottom + 5, btnWid, btnHgt, getText('UI_Cancel'), self,
        DescriptorCreateModal.onClick)
...
function DescriptorCreateModal:close()
    self:destroy()
end

function DescriptorCreateModal:destroy()
...
function DescriptorCreateModal:setJoypadButtons(joypadData)
...
function DescriptorCreateModal:onClick(button)
    ...
    if self.pageLabel then
        ...
        self.pageLabel:setX(self.entry:getRight() -
            getTextManager():MeasureStringX(self.pageLabel.font, self.pageLabel.name))
    end

    self.pinButton:setVisible(false)
end

function DescriptorCreateModal:prerender()
    self.pinButton:setVisible(false)
...
function DescriptorCreateModal:render()
...
function DescriptorCreateModal:onGainJoypadFocus(joypadData)
...
function DescriptorCreateModal:onJoypadDown(button, joypadData)
...
function DescriptorCreateModal:onJoypadDirUp(joypadData)
...
function DescriptorCreateModal:onJoypadDirDown(joypadData)
    if self.joypadIndexY == #self.joypadButtonsY then
        self:clearJoypadFocus(joypadData)
...
function DescriptorCreateModal:new(
    x,
    y,
    width,
    height,
